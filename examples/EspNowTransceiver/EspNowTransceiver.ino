// PCMFlowG711 example: EspNowTransceiver
//
// Half-duplex voice transceiver over ESP-NOW for two (or more) M5Stack
// Core2 boards. The same firmware acts as both sender and receiver:
//
//   while button A is held:
//       M5.Mic (8 kHz mono) -> G711Encoder (mu-law) -> ESP-NOW broadcast
//   always:
//       ESP-NOW recv -> G711Decoder -> PCMFlow -> M5.Speaker
//
// One 20 ms voice frame at 8 kHz / 8 bit = 160 bytes, which fits well
// under ESP-NOW's 250-byte payload limit. The codec adds essentially
// zero CPU on an ESP32, so the loop is dominated by mic/speaker DMA
// and the radio.
//
                         ...
                         int len)
{
    if (len <= 0 || data == nullptr)
        return;
    // Enqueue bytes for PCMFlow's pump() to pull via G711Decoder::readFrames().
